(() => {
  ...
  const configs = {
    friday: {
      ...
      meals: [
        { selector: "#friday-dinner", title: "约21:00 · 四季民福晚饭", note: "同行人已提前排队；到店后直接会合，饭后沿大栅栏—前门—正阳门顺路消食。" }
      ],
      trafficDecisions: [
        ["北京南站 → 首都宾馆", "打车优先", "目标20:35—20:40到酒店", "19:59到站后直接叫能装下实际行李的车；车型不够就分乘，先放行李再去四季民福。", "出站时若导航显示到酒店将晚于20:40，立刻联系排队人，并评估直接带行李去餐厅；今晚不再把地铁当主线。"],
        ["首都宾馆 → 四季民福", "打车优先", "目标20:55前到店", "放下行李后立即出发，终点写全称“四季民福烤鸭店（前门大栅栏店）”。", "20:40仍没完成放行李就不再整理；预计晚于21:00到店，马上通知排队人和门店。"],
        ["四季民福 → 首都宾馆", "步行消食", "约35—45分钟（含顺拍）", "吃完沿大栅栏街—前门大街—正阳门外观向东回酒店，途中看到的夜景都在返程线上。", "22:30后才离店、下雨或明显疲劳，只顺拍前门后打车回酒店。"]
      ],
      journey: [
        { dispatch: 0, traffic: [0], transportDetails: ["#transport-friday .journey-grid > .journey-leg:nth-child(1)"], switches: [0, 1] },
        { dispatch: 1, traffic: [1], transportDetails: ["#transport-friday .journey-grid > .journey-leg:nth-child(2)"], switches: [2] },
        { dispatch: 2, meals: [0], switches: [3] },
        { dispatch: 3, traffic: [2], transportDetails: ["#transport-friday .journey-grid > .journey-leg:nth-child(3)"], details: ["#friday .map-card", "#friday .experience-grid", "#friday .cut-card"], switches: [4] }
      ],
      switches: [
        ["约20:10", "出站先看酒店到达时间", "能装下行李的车预计20:40前到首都宾馆，就先放行李；不要再等地铁换乘。"],
        ["预计晚于20:40到酒店", "保晚饭，不保完整入住流程", "联系提前排队的人；必要时直接去餐厅，或放下行李后不整理、立刻二次出发。"],
        ["20:40", "还没离开酒店", "停止整理，直接打车去四季民福；目标20:55前到店。"],
        ["约21:00", "与排队人会合落座", "四季民福已经是确定主线，不再查看其他餐厅；控制在75—80分钟。"],
        ["22:30后才离店", "缩短夜游", "只沿前门大街顺拍正阳门外观，随后打车回酒店；03:30起床不变。"]
      ],
      checks: [
        ["train", "改签后19:59到北京南", "出站即看网约车预计到达；叫能装下实际行李的车型，装不下就分乘。"],
        ["hotel", "保存首都宾馆完整地址", "前门东大街3号；打车目的地写“首都宾馆正门”，避免导到同名酒店。"],
        ["fridayqueue", "提前排队人保持现场排号", "保持电话畅通；临近叫号时向迎宾说明其余同行人正在赶来，确认是否需要全员到齐。"],
        ["sleep", "周六03:30起床不变", "预计23:00左右回房、23:30左右睡下；今晚的代价是睡眠约4小时。"]
      ],
      sourceLinks: [
        ["嬉游：老城与餐馆原文", "资料归档/00-微信资料台账.md"],
        ["首都宾馆高德位置", "https://ditu.amap.com/place/B000A1A4D2"]
      ],
      copy: "周五 9月11日\n改签后19:59到北京南站。出站直接叫能装下行李的车；预计20:40前到首都宾馆就先放行李，否则立即联系提前排队的人并保住21:00到四季民福。约21:00—22:15吃饭；饭后沿大栅栏街—前门大街—正阳门外观顺路回酒店。22:30后才离店就缩短为顺拍前门后打车。预计23:30睡下，周六03:30起床不变。"
    },
    saturday: {
      ...
      trafficDecisions: [
        ["首都宾馆 → 广场东侧路北安检03", "步行", "约15—25分钟，另算安检排队", "升旗已预约03:30—04:52，核验点已经明确；04:00离店，目标04:20到达。", "不在管制区反复叫车寻找下客点；遇动态分流服从现场人员。"],
        ...
        ["首都宾馆 → 鸟巢4号检票口", "地铁＋步行", "门到门约45—55分钟", "王府井站乘8号线到奥林匹克公园站，出站后按现场导向前往国家体育场北侧4号检票口。", "不预设地铁出口号；演唱会日按现场围挡和工作人员导流行走。"],
      ...
      switches: [
        ...
        ["15:15", "离开酒店的最晚边界", "外卖仍未送到就停止等待；16:40停止奥园外围拍照，直奔4号检票口。"],
        ["22:50", "仍未进入地铁或接驳交通", "删除什刹海；奥园夜景只在疏散过程中顺拍。"],
        ["23:10", "夜宵电话判断", "满恒记能接客、约23:30可落座且等位不超过10分钟才去；否则转北平三兄弟或回酒店。"]
      ],
      checks: [
        ["square", "升旗预约已完成", "09月12日03:30—04:52，广场东侧路北安检03；04:00离开酒店，4人带身份证原件。", true],
        ["palace", "故宫大门票和两馆票均已完成", "9月12日上午场；钟表馆、珍宝馆已经加购。按中轴线＋两馆精品线执行，携带预约证件原件。", true],
        ["concert-seat", "演唱会座位已确认", "L区150通道，一层31排16—19号，4人连座；通过外场安检后按场内指引找150通道。", true],
        ["concert-entry", "4号检票口已确认", "从4号检票口进入；17:00前到达，依次完成实名核验和安检，再去L区150通道。", true],
      ...
      sourceLinks: [
        ...
        ["故宫官方预约购票", "https://www.dpm.org.cn/subject_booking/"],
      ...
      copy: "周六 9月12日\n03:30起床，04:00离店，步行到广场东侧路北安检03；升旗预约时段03:30—04:52，05:52升旗。06:30天兴居、07:00锦芳，07:25去午门。08:30—12:30故宫：中轴线、钟表馆、珍宝馆、御花园；门票均已约。午饭白记优先，13:15未落座切平安锅贴，再切席上喳喳。13:50回房午休，14:25吃外卖，15:05左右离店。17:00前到鸟巢4号检票口，实名核验和安检后找L区150通道；19:00演出。22:50已进入交通才去什刹海；23:10电话确认夜宵。"
    },
    sunday: {
      ...
      trafficDecisions: [
        ...
        ["酒店 / 最后一顿 → 北京南站", "六/七座车或地铁", "约20—35分钟，17:25必须离店", "G49于19:04发车；4人带2只大箱或4只小箱时，普通五座车存在装不下风险。", "优先预约六/七座车；叫不到合适车型或导航预计18:10后到，立即走地铁并取消途中停留。"]
      ],
      journey: [
      ...
      switches: [
        ["09:30", "仍未离开早餐店", "启用颐和园压缩线：删宫廷区细看，不动长廊、佛香阁、石舫乘船和十七孔桥。"],
        ["13:00", "抵达石舫码头", "坐船是已确认主线；正常排队继续等，并从午饭或宫廷区细看回收时间。只有官方停航才改走东堤步行线。"],
      ...
      checks: [
        ...
        ["summerpalace", "颐和园大门票已购买", "4名成人，9月13日上午09:00—12:00入园；东宫门进、新建宫门出不变。", true],
        ["summer-pavilion", "加购佛香阁园中园票", "当前¥120订单对应4张大门票，未显示佛香阁票；佛香阁已确定为必走点，需在原渠道或现场加购。"],
        ["boat", "当天查看游船运行", "石舫—南湖岛段是主线；正常排队不主动放弃。只有官方停航时才改走东堤，并立即压缩后续停留。"],
        ["dinnerqueue", "离园前查看大餐排号", "按周六是否吃到涮肉决定类别，再看预计落座时间；17:25离店优先。"],
        ["train", "核对G49返程车次", "北京南19:04出发、杭州东23:21到达；确认检票口、证件和取行李顺序，18:00前进入车站。"],
      ...
      copy: "周日 9月13日\n07:45起床、退房寄存；早餐后乘4号线到西苑，步行到东宫门，目标10:10入园。宫廷区—长廊—佛香阁—20分钟简餐—石舫乘船或东堤—十七孔桥，14:40新建宫门离园。周六没吃涮肉去满恒记；已吃涮肉则取行李后去烤肉刘。17:25离店，18:00前进北京南站，乘19:04的G49返程。"
    }
  };
  ...
  const restaurantRiskCopy = {
    "四季民福前门大栅栏店": ["今晚已锁定", "同行人提前现场排队；其余人目标20:55到店。临近叫号保持电话畅通，并确认是否需要全员到齐。"],
  ...
  const bindChecks = () => {
    ...
    $$('[data-check]').forEach((input) => {
      ...
      input.checked = confirmed || Boolean(checks[input.dataset.check]);
  ...
  const renderChecks = () => {
    ...
    $("#before-you-go").innerHTML = `
      ...
      <div class="checklist daily-checklist">${config.checks.map(([key, title, body, confirmed]) => `
        <label><input type="checkbox" data-check="${escapeText(key)}"${confirmed ? ' data-confirmed="true" checked disabled' : ""}><span></span><div><b>${escapeText(title)}</b><p>${escapeText(body)}</p></div></label>`).join("")}</div>
      ${links}`;
  };
